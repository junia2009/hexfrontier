...
//
// 遊びが5つに増えて、受付の一文(meets.js の hint)だけでは何をすれば
...
export const GUIDES = {
  ...
  logroll: {
    ms: 90000,
    goal: '沖に浮かぶ<b>でっかい丸太</b>の上で生き残ります。順位は<b>落ちずにいた時間</b>。最後のひとりになったら終わりです。',
    steps: [
      ['🪵', 'のる', 'はじまると、みんな丸太のてっぺんに並んで立たされます。はじめの3秒は回りません'],
      ['🏃', 'あるく', '<b>丸太が回るので横へ転がされます。</b>逆らって歩き続けないと、横から海に落ちます。棒立ちは数秒でおしまい'],
      ['🕳', 'よける', '丸太には<b>切れ目</b>があります。回って上がってくると足場が消えるので、丸太の長さ方向へ歩いて逃げます'],
      ['🌊', 'おちる', '<b>水に触れたらそこで終わり。</b>泳いで戻ることはできず、岸へ戻されます'],
    ],
    tips: [
      '<b>てっぺんから外れるほど滑り落ちます。</b>転がされたら、何よりまずてっぺんへ戻ります',
      '<b>回る速さはだんだん上がって、最後は歩きより速くなります。</b>終盤は誰も踏みとどまれないので、粘った人が勝ちです',
      '切れ目は回ってくるのが見えます。来る前に、切れ目の無いところへずれておきます',
      'よけるのに歩くぶんは、逆らうのに使えません。切れ目を渡るときがいちばん流されます',
    ],
    note: '切れ目の位置と回る向きは<b>その回の種</b>で決まります。同じ回なら全員がまったく同じ丸太に乗っています。',
  },
  daifugo: {
    autoMs: 45000,
...
const SOLO = {
  ...
  dragon: ['🐉 山の上の巣で眠る竜に会う'],
  sea: ['🏝 主島のまわりに浮かぶ5つの小島をながめる(歩いては渡れない)'],
};
const SOLO_ANY = '🎣 港で釣り(釣ったものは図鑑に残る)';
// 店はどの島にもある。**島の上に建っている**ので、歩いて行くものとして書く
const SOLO_SHOP = '🏪 受付の隣の屋台で買い物(遊んでたまった銀貨を使う)';

export function islandSolo(mode) {
  return [SOLO_ANY, SOLO_SHOP, ...(SOLO[mode] ?? [])];
}
...
export function walkGuideHtml() {
  return `<h4>🚶 島のあるきかた</h4>
    ...
    ${stepRow(['🎣', 'つる', '港の桟橋に立つと 🎣 が出ます。'
      + 'アタリで押して合わせ、<b>押している間だけ</b>巻き上げます'])}
    ${stepRow(['🏪', 'かう', '受付の隣の屋台へ行くと 🏪 が出ます。'
      + '店主から、遊んでたまった<b>銀貨</b>で買えます'])}
    <p class="mg-note">上の帯から 🧍 すがた ・ 😀 エモート ・ 🎒 持ち物(買ったもの) ・
    📖 釣り図鑑 ・ ✕ もどる。
...
export function meetGuideHtml(id, opts = {}) {
  ...
  return body
    ...
    : `<h4>🎪 この島に受付はありません</h4>
       <p>いまは<b>どの島にも受付があります</b>。この文が出ているときは、
       島の種類が壊れているか、古い保存から開いています。</p>
       ${walkGuideHtml()}`;
}
