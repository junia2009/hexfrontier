...
// 1〜2時間で全部買えてそこで銀貨が死ぬ。
//
// **6つめは「盤まわり」。** カタン本編の盤で使う見た目の品(gear.js)。
...
const PLACEABLES = DECOR.map((d) => ({
  ...
  kind: 'decor',
  desc: d.desc,
  // **柄があることを、買う前に伝える。** 置くときに選べるので、
  // 色ちがいを買い直す必要が無い ── 書いておかないと気づけない
  note: d.looks?.length
    ? `島のすきな場所に置けます。何個でも買えます。`
      + `柄が${d.looks.length}種類あって、置くときに選べます(${
        d.looks.map((l) => l.name).join('・')})。`
    : '島のすきな場所に置けます。何個でも買えます。',
}));

// ---- 盤まわり(カタン本編の盤で使う見た目の品)----
//
...
export const SHELVES = [
  ...
  },
  {
    ...
    items: PLACEABLES,
    // **飾りだけ2列に並べる。** 品が19もあって、1列だと画面3枚ぶん
    // あった(ほかの棚は4〜6品なので1列のまま読める)。
    // 2列では説明の一行が入らないので、そこは札を押したときに出す。
    grid: true,
  },
  {
    id: 'gear',
