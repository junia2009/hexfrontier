...
export function smooth(rate, dt) {
  return 1 - Math.exp(-rate * dt);
}

// カメラが本人の後ろへ回り込む速さ。**1.6 から落としてある。**
// 行き先はカメラの向きを基準に決めるので、カメラが本人の向きを追うと
// 「カメラが回る → 行き先も回る → もっと回る」と噛み合う。
// 斜めは重みで 0 にできない(下の camFollow)ぶん、速さのほうで抑える。
export const CAM_FOLLOW = 0.35;

// スティックの倒し向きのうち、「奥へ」の成分(0〜1)。
// 真上で 1、真横で 0、手前側は 0。カメラが回り込む重みに使う。
//
// **真横から手前を 0 にするのが要**。ここを 1 のままにしていたので、
// 下に倒しっぱなしにすると本人もカメラも回り続けていた
// (実測 2 秒で 570° ── 1.6 周)。
export function camFollow(input) {
  const mag = Math.hypot(input.x, input.y);
  if (!(mag > 1e-6)) return 0;
  return Math.max(0, input.y / mag);
}
