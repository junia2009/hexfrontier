...
export const SPECIES = [
  ...
  },
  {
    ...
    props: {
      ...
      upperArm: 0.036, foreArm: 0.032, armR: 0.030, handR: 0.033,
      bodyR: 0.082,
      // 目・ほっぺ・口は白い顔の**上に載せる**。既定の位置のままだと
      // 白い顔の中に沈む ── 目は既定 z=0.80R で前端が 0.955R、白い顔の面は
      // その高さで 0.964R なので、わずかに埋まって黒目が削れていた
      // (帽子と目がつながって、目がほとんど見えなかった原因)。
      eye: { r: 0.235, x: 0.415, y: 0.02, z: 0.92 },
      blush: { r: 0.18, x: 0.56, y: -0.26, z: 0.92 },
      // 口は嘴の上に線として置く。嘴の下に別に口を描くと、山羊ひげのような
      // 黒い塊が付いて見えた。嘴の合わせ目に重ねれば、驚いたときは嘴が開く。
      mouth: { r: 0.10, tube: 0.018, y: -0.16, z: 1.16 },
    },
    // 手足も鳥のものに替える(ミトンの腕と靴のままだと、色を塗っても
    // ペンギンに見えない)。白い顔 + 黒い帽子で顔の輪郭を作る。
    parts: {
      beak: true, mask: true, belly: true, flipper: true, webbed: true, tail: 'wedge',
    },
  },
  {
    id: 6,
