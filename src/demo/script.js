...
// していたが、それでは動画にする意味がない ── エントリーして実際に始める。
//
// **始めたら自動運転に任せる**(`{ auto: true }`)。ビートごとに1手だけ
// 出していたころは、説明を読んでいるあいだ誰も動かず、大会だけが進んで
// いた ── 実測で、大富豪は自分の手番のまま 12 秒 + 21 秒固まり、
// 丸太乗りは 4.1 秒で落ちて残り4ビートを岸で喋り、竜からは 9.9 秒で
// 捕まって最下位だった。**字幕を読んでいる間も遊び続けるのが筋。**
function meetIntro(id, name, lead, intro) {
  return [
    ...
    },
    {
      ...
      // auto はここから章の終わりまで裏で回り続ける(待たない)
      island: { meet: { cpu: 3 }, auto: true },
      hold: 1800,
    },
  ];
}
...
const fishMeetBeats = [
  ...
  {
    say: '小さい魚でも合計に足されるので、大物を粘るか数を釣るかは自由です。時間いっぱい投げ続けます。',
    island: { wait: 3600 },
    hold: 3600,
  },
  meetOutro,
...
const dragonHuntBeats = [
  ...
  {
    // **竜の話は、竜が追っているあいだに言い切る。** 3つに分けていたころは、
    // 最後のひとりになった時点で回が終わる(実測で 28〜40 秒)ので、
    // 後ろのビートが結果の表の上で流れていた。2つにまとめて尺を合わせる。
    say: '竜は歩きより少し遅い代わりに、曲がるのが下手です。まっすぐ逃げず、斜めに振ってやり過ごします。',
    island: { wait: 1800 },
    hold: 1800,
  },
  {
...
const logrollBeats = [
  ...
  {
    // **丸太の話は、丸太が映っているあいだに言い切る。** 4つに分けて
    // いたころは、最後まで残ると回がそこで終わる(ひとりになったら終わり)
    // ので、後ろ2つが岸の結果画面の上で流れていた ── 実測で、回は 25 秒で
    // 終わるのに章は 46 秒あった。中身を2つにまとめて、尺を回に合わせる。
    say: '丸太が回るので横へ転がされます。てっぺんから外れるほど滑り落ちるので、逆らって歩き続けます。',
    island: { wait: 1400 },
    hold: 1400,
  },
  {
  ...
  {
    // ここは結果が出たあとに流れる。**残った人が勝ち**の話なので、
    // 順位の表と並んで困らない
    say: '回る速さはだんだん上がって、最後は歩きより速くなります ── 最後まで残った人が勝ちです。',
    island: { wait: 2400 },
    hold: 2400,
  },
  meetOutro,
...
const raidMeetBeats = [
  ...
  {
    // **櫓まで歩く。** ここを入れるまで、字幕が「櫓に登ると」と言っている
    // のに人は受付の前に立ったままで、受付のパネルが画面の下半分を
    // 覆ったままだった(実測: 櫓までの距離 4.3 のまま・点は 0 のまま)。
    // 受付から離れればパネルも閉じる。
    say: '始まったら浜の櫓へ。登ると弓を構えます。',
    island: { walk: 'post' },
    hold: 1600,
  },
  {
    say: '押している間、弓を引き絞り ── 離すと放ちます。',
    island: { wait: 2600 },
    hold: 2600,
  },
  {
