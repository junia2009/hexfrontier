...
- **対戦の緊張は「段」で切り替える**(`raceScene`)。いちばん進んでいる人が
  ...
  | 段 | 旋法 | 進行 | 和音 | 音域 | 刻み | 旋律 |
  |---|---|---|---|---|---|---|
  | 平常 | ドリアン | grand | 5.6秒 | — | なし | 0.6 |
  | 接近 | ドリアン | drive | 3.9秒 | — | 0.9 | 0.2 |
  | 王手 | エオリア | drive | 3.2秒 | 1oct下 | 1.0 | 0.1 |

  **濃さを 0〜1 で連続に動かすだけでは耳に届かない。** 最初はそう作ったが、
  実際に遊んで「全く分からない」となった。原因は測って分かった:
  旋法・和音進行・音域が最後まで同じで、動いていたのは音量バランスと速さだけ
  (しかも `base + add` が 1 を超えるパートは頭打ちで、宣言したぶん動いていなかった)。
  人は勾配ではなく出来事に気づくので、段で切り替えて曲想ごと入れ替える。

- **段が耳に届く大きさか、実際の波形で確かめる。** 譜面の数字ではなく、
  `master` に解析器を挿して重心(明るさ)と音量の揺れ(粒立ち)を測る。
  基準は「すでに分かると確認できている切り替え」= 散策 → 各ミニゲーム。
  平常 → 王手 は 揺れ 1.73倍 / 重心 1.32倍 で、どの基準の切り替えより大きい。
  測りかたには落とし穴が2つあった ──
  立ち上がりの数え上げは `requestAnimationFrame` の間隔(3D描画に律速されて
  7fps しか出ない)に左右されて誤った結論を出す。`setInterval` で等間隔に
  取り直すこと。しきい値も勘で置かない: 基準の切り替えを測ると
  散策 → 蛮族 は揺れ 1.15倍しかなく、勘で置いた 1.3倍は基準自身が通らなかった。
- **盛り上げたい段ほど持続音は減らす。** 「全部を最大にする」と低音が刻みを
  覆って、実測では王手のほうが接近より拍が聞こえなくなっていた。
  張り詰めた感じは音の量ではなく粒立ち(刻みと撥弦)から出る。
