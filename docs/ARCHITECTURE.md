...
│       ├── logroll-contest.js # 丸太乗り。筏の種と浮かべた場所を配る
...
| 航海者たち | 丸太乗り | 筏の**種**と浮かべた場所を配る。**落ちたのはクライアントが申告**(時刻はサーバーが打つ) |
...
| 丸太乗り | 沖に浮かぶ筏(航海者たち) | `logroll.js` / `logroll-fx.js` |
...
- **丸太乗りは「動く足場」を地面の関数として出している。**
  ...
  - **丸太は全部同じ向きに回す。** 隣どうしを逆に回すと、あいだに
    「押し合って動かない谷」ができて、立っているだけで落ちなくなった
    (実測: 8秒で 0.18 タイル)。そろえると筏ぜんたいが一方向へ流れる帯になり、
    流れに逆らって足踏みし続ける遊びになる ── 本物の丸太乗り(birling)と同じ。
  ...
  - **見た目の回転は THREE の回り方に合わせる。** `Object3D.rotation.y` は
    `logroll.js` の `toWorld` と逆回りなので、筏の向きには `-anchor.angle` を
