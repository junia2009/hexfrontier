...
- **丸太乗りは「動く足場」を地面の関数として出している。**
  ...
  - **軸を海面に置く。** 丸太は半分沈み、水から出ているところがそのまま
    足場になるので、足場の切れる角と水に落ちる場所が一致する
  ...
  - **回る速さは上げていく。** 一定だと 90 秒がにらみ合いになって誰も
    落ちない。速さが変わるので、回った角は `spin × t` ではなく**積分**
    (`turnAt`)で持つ ── でないと速さを変えた瞬間に丸太が飛ぶ。
