...
- **光る苔**(`makeMossMaterial` / `addMoss` / `buildMoss`): 夜の道しるべ。
  砂漠以外の全ヘックスに小さな粒を散らし、`emissive` を `_tickSky` が
  夜の濃さ(`night`)× `MOSS_GLOW` で動かす ── 昼は 0 なので暗い苔の緑のまま。
  島じゅうで 1000 を超えるので **1 つの `InstancedMesh` にまとめる**(描画呼び出し 1 回)。
  粒は離して散らすこと。重ねて平たく潰すと隣どうしが溶けて 1 枚の面になり、
  苔ではなく塗料をこぼした跡に見える。
