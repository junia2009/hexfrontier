...
// 上のテストは waveSeed を直に呼んでいるので、**setSeed が受け取った種を
// 捨てていても通ってしまう**。実際 `this.base = makeRng(Number(seed) || 1)` の
// `||` を `&&` にすると種が常に 1 になり、毎回まったく同じ波が出るのに、
// このファイルの18本が全部通った。器を通して2つの種を突き合わせる。
test('射る大会: setSeed で渡した種が波に届いている', () => {
  assert.notEqual(
    started(11).view(at(0)).seed,
    started(22).view(at(0)).seed,
    'setSeed の種が波に届いていない(種を捨てて定数で回している)',
  );
});
